...
**Still open from this list:** item 2 (image OCR failing to extract text — needs its own investigation, likely a media-download or vision-call issue, not yet looked at), items 5+6 (short-term conversational memory for clarifying replies, and forwarded-message sender-as-default-audience — a real feature needing its own design pass, next per Roy's stated priority), item 7 (color picker UI applying a different shade than what was shown/picked — frontend-only, not yet looked at).
