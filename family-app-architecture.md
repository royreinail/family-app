...
| Body | `⏰ {{reminder_text}} — Family App` |
| Sample for `{{reminder_text}}` | `Reminder: pick up the dry cleaning at 5pm` |
| Footer / Buttons | none |

Revised once against Meta's real form: WhatsApp Manager now requires a *named* variable (lowercase + underscores, e.g. `{{reminder_text}}`), not the older positional `{{1}}`, and rejects a variable placed at the very start or end of the body — hence the surrounding text above. The body's own fixed text still deliberately carries no "Reminder:" of its own (only the emoji + trailing "— Family App") because `reminders.js`'s `scheduleReminder` already builds `Reminder: {title}` into the text passed in as `{{reminder_text}}` — a template-side "Reminder:" too would double it up. `messenger.js`'s `sendTemplate()` sends the matching `parameter_name: "reminder_text"` on the API call, not a positional-only parameter, so it must stay in sync with whatever name is actually approved.
